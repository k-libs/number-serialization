...
/**
 * Returns a [ByteArray] containing the 8 bytes that make up the target `Long`
 * value.
 *
 ...
 * that make up the target `Long` value.
 *
 ...
 */
@OptIn(ExperimentalUnsignedTypes::class)
fun Long.getUBytes(buffer: UByteArray, offset: Int = 0, be: Boolean = true) = getBytes(buffer.asByteArray(), offset, be)

private inline fun Long.unsafeGetBytes(buffer: ByteArray, offset: Int, be: Boolean) {
  if (be) {
    buffer[offset    ] = ((this shr 56) and 0xFF).toByte()
    buffer[offset + 1] = ((this shr 48) and 0xFF).toByte()
    buffer[offset + 2] = ((this shr 40) and 0xFF).toByte()
    buffer[offset + 3] = ((this shr 32) and 0xFF).toByte()
    buffer[offset + 4] = ((this shr 24) and 0xFF).toByte()
    buffer[offset + 5] = ((this shr 16) and 0xFF).toByte()
    buffer[offset + 6] = ((this shr 8 ) and 0xFF).toByte()
    buffer[offset + 7] = ((this       ) and 0xFF).toByte()
  } else {
