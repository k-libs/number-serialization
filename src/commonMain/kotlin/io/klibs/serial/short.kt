...
/**
 * Returns a [ByteArray] containing the 2 bytes that make up the target `Short`
 * value.
 *
 ...
 * that make up the target `Short` value.
 *
...
fun Short.getUBytes(be: Boolean = true): UByteArray {
 ...
 */
@OptIn(ExperimentalUnsignedTypes::class)
fun Short.getUBytes(buffer: UByteArray, offset: Int = 0, be: Boolean = true) = getBytes(buffer.asByteArray(), offset, be)

private inline fun Short.unsafeGetBytes(buffer: ByteArray, offset: Int, be: Boolean) {
  val v = this.toInt()
  if (be) {
