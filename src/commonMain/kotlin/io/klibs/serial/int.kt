...
/**
 * Returns a [ByteArray] containing the 4 bytes that make up the target `Int`
 * value.
 *
 ...
 * that make up the target `Int` value.
 *
 ...
 */
@OptIn(ExperimentalUnsignedTypes::class)
fun Int.getUBytes(buffer: UByteArray, offset: Int = 0, be: Boolean = true) = getBytes(buffer.asByteArray(), offset, be)

private inline fun Int.unsafeGetBytes(buffer: ByteArray, offset: Int, be: Boolean) {
  if (be) {
    buffer[offset    ] = ((this shr 24) and 0xFF).toByte()
    buffer[offset + 1] = ((this shr 16) and 0xFF).toByte()
    buffer[offset + 2] = ((this shr 8 ) and 0xFF).toByte()
    buffer[offset + 3] = ((this       ) and 0xFF).toByte()
  } else {
